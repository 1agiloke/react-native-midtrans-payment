package com.knaxreactnativemidtrans
...
class ReactNativeMidtransModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
  ...
  fun checkout(options: ReadableMap, promise: Promise) {
    ...
    if (selectedPaymentMethod == -1.0) {
      ...
      MidtransSDK.getInstance().startPaymentUiFlow(currentActivity, PaymentMethod.values()[selectedPaymentMethod.toInt()]);
    }
  }
}
